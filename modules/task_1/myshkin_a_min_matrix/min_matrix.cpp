...
/*int getSequentialMinVector(const std::vector<int> vect, const int n) {
	int min = vect[0];
	for (int i = 0; i < n; i++) {
		if (min > vect[i]) {
			min = vect[i];
...
int getParallelMinMatrix(const std::vector<int> miv, int rows, int cols) {
	...
	if (rank == 0) {
		...
		min = getSequentialMinMatrix(miv, 1, delta + remainder);
		//min = getSequentialMinVector(miv, delta + remainder);
	} else {
		MPI_Status Status;
		MPI_Recv(&buf1[0], delta, MPI_INT, 0, 0, MPI_COMM_WORLD, &Status);
		min = getSequentialMinMatrix(buf1, 1, delta);
		//min = getSequentialMinVector(buf1, delta);
	}
